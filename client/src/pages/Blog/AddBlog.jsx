...
const AddBlog = () => {
  ...
  const formSchema = z.object({
    ...
    blogContent: z.string().min(1, "Blog Content is requried"),
  });
  ...
  async function onSubmit(values) {
    try {
          ...
          // Validate editor content: strip html and ensure there's actual text (Add by AI)
          const stripHtml = (html = '') => html.replace(/<[^>]*>/g, '').replace(/&nbsp;|\s/g, '')
          if (!newValues.blogContent || stripHtml(newValues.blogContent).length === 0) {
            return showToast('error', 'Blog content is required')
          }

          if(!file){
            return showToast('error', 'Feature image requried')
          }
        ...
        <CardContent>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)}>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="Slug Address" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        {/* Pass props directly so Editor receives onChange and initialData as expected */}
              ...
              <Button
                type="submit"
                className="w-full bg-violet-700 hover:bg-violet-700"
              >
                Submit
              </Button>
            </form>
