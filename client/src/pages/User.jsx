              ...
              {data && data.user.length > 0 ? 
                data.user.map((user) => (
                  <TableRow key={user._id}>
                    ...
                    <TableCell><img src={user.avatar || usericon} className="w-8 rounded-full"/></TableCell>
                    <TableCell>{moment(user.createdAt).format('DD-MM-YYYY')}</TableCell>
                    
                    <TableCell className='flex gap-3'>
                    
                     <Button onClick={()=> handleDelete(user._id)} className='hover:bg-violet-600 hover:text-white'variant="outline">
                      <FaTrashAlt/>
                     </Button>
                    </TableCell>
                  </TableRow>
